	...
	<script type="text/javascript">
		function detalles(idM){
			...
			$.post('Movimiento',{	
				...
				var persona = response ['object']['nombrePersona'];
				
				$('#articuloMosT').text(nomA);
				$('#medidaMosT').text(medida);
				$('#cantidadMosT').text(cantidad);
				$('#fechaMosT').text(fecha);
				$('#obsMosT').text(observacion.trim());
	...
	<div class="container">
		...
		<div class="row" >
			<div class="col-xs-4  col-xs-offset-1">
				<h3><b>REPORTE DE MOVIMIENTOS</b></h3>
			</div>
				...
				<table class="table" id="inven">
					...
					<%for(int i=0;i<movimientos.size();i++){ %>
						<tr>
							...
							<td><%=movimientos.get(i).getNombrePersona() %><input type="hidden" name="persona" value="<%=movimientos.get(i).getId_usuario()%>"></td>
							<td><%=movimientos.get(i).getFecha_mov() %></td>
							<td><a href="" onclick="detalles(<%=movimientos.get(i).getId()%>);" data-target="#modDetMT" data-toggle="modal" data-id="<%=movimientos.get(i).getId()%>">Ver</a></td>
						</tr>
					<%} %>
					</tbody>
				</table>
